...
async function obtenerCatalogo() {
  const { data, error } = await supabaseClient
    .from("productos")
    .select("id, nombre, imagen_url, orden, variantes(id, sku, modelo, precio_actual, precio_anterior, stock_estado, stock_cantidad, activo)")
    .eq("activo", true)
...
async function obtenerCatalogoCompleto() {
  const { data, error } = await supabaseClient
    .from("productos")
    .select("id, nombre, imagen_url, activo, orden, variantes(id, sku, modelo, descripcion_completa, precio_actual, precio_anterior, stock_estado, stock_cantidad, activo)")
    .order("orden", { ascending: true })
    .order("nombre", { ascending: true });
